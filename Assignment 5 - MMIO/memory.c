...
int initialize(int proc_id, const char* instruction_filename, const char* memory_filename){
    ...
    /* Read only USER_DATA_SIZE bytes — the MMIO page has no file backing. */
    FILE *dptr = fopen(memory_filename, "r");
    if(!dptr){
        printf("Error opening data file %s\n", memory_filename);
        return 0;
    }
    static char data_buf[USER_DATA_SIZE];          /* was DATA_MEM_SIZE */
    int m = 0;
    while(m < USER_DATA_SIZE && fscanf(dptr, "%2x", &val) == 1){
        data_buf[m++] = (char)val;
    }
    fclose(dptr);
    ...
    /* Map the MMIO logical page to MMIO_FRAME. This does not consume a
       physical RAM frame; the "frame" is a magic number the byte-access
       routines interpret as "route to the device layer". */
    int mmio_page = 1024 / PAGESIZE + MMIO_DATA_OFFSET / PAGESIZE;   /* page 10 */
    memory[pt_frame * PAGESIZE + mmio_page * PTE_SIZE] = (unsigned char)MMIO_FRAME;
    ...
    printf("MMU: proc %d loaded %d instr bytes (%d page%s) + %d data bytes (%d page%s), PT in frame %d, MMIO on page %d\n",
           proc_id, n, instr_pages, instr_pages == 1 ? "" : "s",
           m, data_pages, data_pages == 1 ? "" : "s", pt_frame, mmio_page);
    return 1;
}
...
int count_pages_needed(const char* instruction_filename, const char* memory_filename){
    ...
    int data_pages = pages_needed_for(m, USER_DATA_SIZE / PAGESIZE);
...
void finalize(int proc_id, const char* memory_filename){
    ...
    if(!dptr){
        ...
        /* Write back only the user data region — the MMIO page's "contents"
           are device state, not task state. */
        for(int i = 0; i < USER_DATA_SIZE; i += 4){        /* was DATA_MEM_SIZE */
    ...
    for(int page_index = 0; page_index < NUM_LOGICAL_PAGES; page_index++){
        ...
        /* Skip unmapped pages and MMIO_FRAME — the latter is not a RAM
           frame and must never be returned to the free pool. */
        if(frame != 0 && frame != MMIO_FRAME){
            freePage(frame);
        }
        memory[pte_addr] = 0;
    }
...
int read_word(int proc_id, int i){
    ...
    /* MMIO: still translate first so an unmapped MMIO page faults,
       then route the whole word to the device layer. */
    if(i >= MMIO_DATA_OFFSET){
        int page_index = i / PAGESIZE + 1024 / PAGESIZE;
        int frame = translate_page(proc_id, page_index);
        if(frame != MMIO_FRAME){
            printf("ERROR: proc %d MMIO read at %d mapped to frame %d (expected %d)\n",
                   proc_id, i, frame, MMIO_FRAME);
            return 0;
        }
        return dev_read(i - MMIO_DATA_OFFSET);
    }

    /* Normal data region: byte-by-byte, page-straddle-safe (unchanged) */
    unsigned int val = 0;
    for(int b = 0; b < 4; b++){
...
void write_word(int proc_id, int i, int val){
    ...
    if(i >= MMIO_DATA_OFFSET){
        ...
        int frame = translate_page(proc_id, page_index);
        if(frame != MMIO_FRAME){
            printf("ERROR: proc %d MMIO write at %d mapped to frame %d\n",
                   proc_id, i, frame);
            return;
        }
        dev_write(i - MMIO_DATA_OFFSET, val);
        return;
    }
    ...
    for(int b = 0; b < 4; b++){
        int logical_addr = i + b;
        int page_index = logical_addr / PAGESIZE + 1024 / PAGESIZE;
        int frame = translate_page_alloc(proc_id, page_index);
        int phys = frame * PAGESIZE + (logical_addr % PAGESIZE);
