...
int getPhysicalAddress(int proc_id, int isFetch, int address){
    ...
    /* PT walk: Find the frame holding this task's page table 
       (the OS-side stand-in for a PTBR register), compute the
       byte address of the PTE, and read it out of physical memory. 
       This is a real memory access, not a C array lookup - which is 
       exactly the extra cost a TLB exists to hide. */
    ...
    int frame = memory[pte_addr]; // actual memory read
    if(frame == 0){
...
int read_word(int proc_id, int i){
    ...
    for(int b = 0; b < 4; b++){
        ...
        int frame = memory[pt_frame * PAGESIZE + page_index * PTE_SIZE]; // PTE read
...
void write_word(int proc_id, int i, int val){
    ...
    for(int b = 0; b < 4; b++){
        ...
        if(memory[pte_addr] == 0){
            int frame = getFreePage();
            memory[pte_addr] = (unsigned char)frame; // PTE write
            memset(&memory[frame * PAGESIZE], 0, PAGESIZE);
        }

        int phys = getPhysicalAddress(proc_id, 0, logical_addr); // now guaranteed mapped
